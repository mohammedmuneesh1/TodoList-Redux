...
const todoslice = createSlice({
  ...
  reducers: {
    ...
    addtask: (state, action) => {
      const LID = state.Ttask.reduce(
        (total, value) => Math.max(total, value.id),
        0
      );
      const NewItem = {
        id: LID + 1,
    ...
    deletetask: (state, action) => {
      const updatedTasks = state.Ttask.filter(
        (value) => value.id !== action.payload
      );
      state.Ttask = updatedTasks;
    },
    completedtask: (state, action) => {
      const completed = state.Ttask.find(
        (value) => value.id === action.payload
      );
...
export const { ThemeChange, addtask, edittask, deletetask, completedtask } =
  todoslice.actions;
